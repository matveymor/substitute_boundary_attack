...
class BiasedBoundaryAttack(MinimizationAttack):
    ...
    def __init__(
        ...
        #self.perlin_param = perlin_param,
        self.distances_iter = np.zeros(steps)
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        for step in range(1, self.steps + 1):
            ...
            perlin_noise = ep.astensor(torch.tensor([get_perlin(originals.numpy()[0].transpose((1, 2, 0)), perlin_param)]).to('cuda'))
            #-----------END-----------------
            ...
            self.distances_iter[step-1] = ep.norms.l2(flatten(initial_pict - best_advs)).numpy()/(3*32*32)
...
def get_perlin(x, param):
    return create_perlin_noise(px=x.shape[0], color=True, batch_size=1, 
                               normalize=True, freq=param, precalc_fade=calc_fade(x.shape[0], freq=param))[0]