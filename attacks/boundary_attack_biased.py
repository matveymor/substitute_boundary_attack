...
class BoundaryAttack(MinimizationAttack):
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        device = model.device
        train_step = 500
        self.metrics = np.zeros(self.steps)
        ...
        for step in tqdm(range(1, self.steps + 1)):
            ...
            if (step%train_step==0) and (step>0) and (len(self.class_1)) and (len(self.class_2)):
                ...
                class_1 = np.array([image.numpy()[0] for image in class_1 if image.shape[1]==shape[1]])
                class_2 = np.array([image.numpy()[0] for image in class_2 if image.shape[1]==shape[1]])
            ...
            self.metrics[step-1] = distances.numpy()[0]
...
def get_projected_gradients(x_current, x_orig, label, surrogate_model):
    ...
    projected_gradient = mask * projected_gradient
